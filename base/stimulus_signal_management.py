...
_STIMULUS_ROW_COLUMNS = tuple(model_consts.DB_STIMULUS_COLUMNS)
...
_FREQUENCY_STEPPED_DB_OWNED_PAYLOAD_KEYS = (
    "stimulus_id",
    "stimulus_name",
    "is_default",
    "voltage_type",
...
def _merge_frequency_stepped_noncanonical_metadata(authoritative_metadata, caller_metadata):
    ...
    for key, value in caller_metadata.items():
        if key in _FREQUENCY_STEPPED_CANONICAL_METADATA_KEYS:
            continue
        metadata[key] = value
...
def _stimulus_row_to_dict(row):
    if isinstance(row, dict):
        result = dict(row)
    else:
        result = {}
        for index, column in enumerate(_STIMULUS_ROW_COLUMNS):
...
def _require_frequency_stepped_db_save_resolution(metadata):
    ...
    if frequency_mode != "octave":
        return
    metadata["resolution"] = _validate_resolution(metadata.get("resolution"))


def _require_frequency_stepped_db_load_resolution(metadata):
    frequency_mode = _valid_frequency_mode_value(metadata.get("frequency_mode"))
    stimulus_type = _valid_frequency_mode_value(metadata.get("stimulus_type"))
    if (frequency_mode or stimulus_type) != "octave":
        return
    metadata["resolution"] = _validate_resolution(metadata.get("resolution"))


def _parse_frequency_stepped_row(row):
    parsed = dict(row)
    ...
    for key in _FREQUENCY_STEPPED_DB_OWNED_PAYLOAD_KEYS:
        if key in parsed:
...
def _query_rows_to_dicts(query_data):
    return [_parse_frequency_stepped_row(_stimulus_row_to_dict(row)) for row in query_data]


def _frequency_stepped_insert_values(stimulus_info, is_default):
        ...
        try:
            ...
            with DataSave(model_consts.DATABASE_PATH) as database:
                ...
                if normalize_stimulus_method(stimulus_info.get("stimulus_method")) == "frequency_stepped":
                    insert_stimulus_config = _frequency_stepped_insert_values(stimulus_info, is_default)
