...
class AudioThdFrequencyResponseAnalysis(object):
    ...
    def calculate_fr(reference_signal, recorded_signal, sr, is_smooth=True):
        ...
        idx_2 = np.argmax(
            np.abs(
                np.fft.fft(reference_signal[mid_slice - (slice_length // 2) - 1 : mid_slice + (slice_length // 2) + 1], num)[
                    : num // 2
                ]
            )
        )
    ...
    def spl_calculation(
        ...
        if method == "envelope":
            ...
            amplitude_list = uniform_filter1d(amplitude_envelope, size=window_size, axis=0, mode=mode, cval=cval)
        else:
            amplitude_list = np.sqrt(uniform_filter1d(signal_float**2, size=window_size, axis=0, mode=mode, cval=cval))
