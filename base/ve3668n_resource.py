...
class VeRecordingAdapter:
    ...
    def _bind_timeout(self, message):
        with self._lock:
            self._pending_started_at = None
            self._diagnostics.append("bind: " + message)
            self._detached.set()
            self._bound.set()
    ...
    def _detach_timeout(self, message):
        with self._lock:
            ...
            self._diagnostics.append("detach: " + message)

    def _confirm_detached(self):
        self._detached.set()

    def _record_failure(self, stage, message):
        with self._lock:
            self._pending_started_at = None
...
class VeResourceController:
    ...
    def _detach(self, adapter, timeout=None):
        ...
        if not adapter._detached.wait(timeout):
            ...
            self._mark_failed("detach", message)
            return False
        return adapter.handles_released

    def _mark_failed(self, stage, message, adapter=None):
        callback = None
        with self._lock:
            self._diagnostics.append(f"{stage}: {message}")
        ...
        if adapter is not None:
            adapter._record_failure(stage, message)
    ...
    def _owner_loop(self):
        while True:
            ...
            try:
                ...
                self._mark_failed("read_task_data", str(exc), read_adapter)
                return
    ...
    def _initialize(self, adapter):
        ...
        try:
            ...
            self._mark_failed(stage, str(exc), adapter)
            return False
