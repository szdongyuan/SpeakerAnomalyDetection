...
from consts.frequency_stepped_consts import (
    ...
    FREQUENCY_STEPPED_FADEOUT_TAIL_DURATION_S,
    FREQUENCY_STEPPED_FADEOUT_TAIL_EXPONENT,
    FREQUENCY_STEPPED_LABEL,
    FREQUENCY_STEPPED_METHOD,
    FREQUENCY_STEPPED_MODES,
    FREQUENCY_STEPPED_RESOLUTIONS,
    FREQUENCY_STEPPED_SCHEDULE_ALGORITHM,
...
def build_frequency_stepped_frequencies(
    *,
    ...
    resolution_value = (
        validate_frequency_stepped_resolution(resolution)
        if mode == "octave" and resolution is not None
        else None
    )
...
def generate_frequency_stepped(
    *,
    ...
    fadeout_tail_sample_count = int(round(sample_rate * FREQUENCY_STEPPED_FADEOUT_TAIL_DURATION_S))
    ...
    schedule_provenance = {
        ...
        "schedule_algorithm": FREQUENCY_STEPPED_SCHEDULE_ALGORITHM,
    }
    if frequency_result.resolution is not None:
        schedule_provenance["resolution"] = frequency_result.resolution

    metadata = {
        "stimulus_method": FREQUENCY_STEPPED_METHOD,
        "stimulus_label": FREQUENCY_STEPPED_LABEL,
        "frequency_mode": frequency_result.frequency_mode,
        ...
        "fadeout_tail_duration_s": FREQUENCY_STEPPED_FADEOUT_TAIL_DURATION_S,
        "fadeout_tail_exponent": FREQUENCY_STEPPED_FADEOUT_TAIL_EXPONENT,
...
def _repair_resolver_metadata(stimulus_metadata):
    ...
    mode = valid_frequency_stepped_mode_value(repaired.get("frequency_mode"))
    stimulus_type = valid_frequency_stepped_mode_value(repaired.get("stimulus_type"))
    if mode is None:
...
def _resolver_retained_frequencies(stimulus_metadata):
    ...
    try:
        ...
        if valid_frequency_stepped_mode_value(stimulus_metadata.get("frequency_mode")) == "octave":
...
def _resolver_valid_resolution_or_none(stimulus_metadata):
    ...
    if resolution is None:
        return None
    try:
        return validate_frequency_stepped_resolution(resolution)
    except (TypeError, ValueError):
        return None
...
def _build_fadeout_tail(*, sample_rate, sample_count, final_frequency, terminal_phase, amplitude):
    ...
    gain = (1 - n / sample_count) ** FREQUENCY_STEPPED_FADEOUT_TAIL_EXPONENT
...
def _cached_schedule_result(stimulus_metadata, sample_rate):
    ...
    if stimulus_metadata.get("schedule_sample_rate") != sample_rate:
        return None
    if provenance.get("schedule_algorithm") != FREQUENCY_STEPPED_SCHEDULE_ALGORITHM:
        return None
...
def _apply_resolver_scalar_bounds(stimulus_metadata, metadata):
    ...
    mode = valid_frequency_stepped_mode_value(metadata.get("frequency_mode"))
...
def _build_octave_frequencies(*, start_freq, stop_freq, sample_rate, safe_max_freq, resolution):
    resolution = validate_frequency_stepped_resolution(resolution)
    start = _positive_float(start_freq, "start_freq")
    stop = _positive_float(stop_freq, "stop_freq")
    clamped_start = min(start, safe_max_freq)
    clamped_stop = min(stop, safe_max_freq)
...
def _preferred_frequencies(resolution, safe_max_freq):
    if resolution == "R3":
        ...
        for decade in _preferred_decades(safe_max_freq):
            scale = 10**decade
            r10_values.extend(
                base * scale for base in OCTAVE_PREFERRED_BASE_FREQUENCIES["R3"]
            )
        ...
        for decade in range(-2, 6):
            scale = 10**decade
            values.extend(base * scale for base in OCTAVE_PREFERRED_BASE_FREQUENCIES[resolution])
...
def valid_frequency_stepped_mode_value(value):
    if value is None:
        return None
...
def validate_frequency_stepped_resolution(resolution):
    if not isinstance(resolution, str):
        raise ValueError(f"Unsupported resolution: {resolution}")
    resolution_value = resolution.strip()
    if resolution_value not in FREQUENCY_STEPPED_RESOLUTIONS:
        raise ValueError(f"Unsupported resolution: {resolution}")
...
def _resolve_frequency_mode(frequency_mode, stimulus_type):
    ...
    if (
        ...
        and stimulus_type_value in FREQUENCY_STEPPED_MODES
        and mode != stimulus_type_value
    ):
        raise ValueError("frequency_mode and stimulus_type conflict")
    if mode not in FREQUENCY_STEPPED_MODES:
        raise ValueError(f"Unsupported frequency_mode: {mode}")
    return mode
...
def _all_preferred_frequencies(max_frequency):
    preferred = set()
    for resolution in FREQUENCY_STEPPED_RESOLUTIONS:
