...
def export_channel_mapping_csv(
    storage_context,
    wav_stem,
    channel_mapping,
    channel_labels,
):
    """Persist the physical-channel labels used by this recording's report."""

    path = build_wav_csv_directory(storage_context, wav_stem) / "channel_mapping.csv"
    if path.is_file():
        return CsvExportRecord("通道映射", True, str(path))
    labels = {
        str(channel or "").strip().upper(): str(label or "").strip()
        for channel, label in dict(channel_labels or {}).items()
    }
    rows = []
    for mapping in sorted(channel_mapping, key=lambda item: int(item.raw_channel)):
        channel = f"CH{int(mapping.raw_channel) + 1}"
        rows.append((channel, labels.get(channel) or "未配置"))
    try:
        _write_csv_atomic(path, ("physical_channel", "label"), rows)
    except (OSError, TypeError, ValueError, csv.Error) as error:
        return CsvExportRecord("通道映射", False, error_message=str(error))
    return CsvExportRecord("通道映射", True, str(path))
