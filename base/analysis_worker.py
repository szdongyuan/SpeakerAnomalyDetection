...
def _execute_task(request, audio, sample_rate, event_queue, log_queue):
    ...
    if storage_context is not None:
        mapping_record = export_channel_mapping_csv(
            storage_context,
            Path(request.wav_path).stem,
            request.channel_mapping,
            channel_labels,
        )
        _emit_log(
            log_queue,
            "INFO" if mapping_record.ok else "WARNING",
            (
                "analysis_channel_mapping_saved"
                if mapping_record.ok
                else "analysis_channel_mapping_save_failed"
            ),
            request,
            artifact_path=mapping_record.file_path,
            error_message=mapping_record.error_message,
        )
