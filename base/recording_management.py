...
_FREQUENCY_STEPPED_RECORDING_MATCH_KEYS = FREQUENCY_STEPPED_CANONICAL_METADATA_KEYS | {
    "voltage_type",
    "voltage",
}
...
class RecordingManager(object):
    ...
    def _get_frequency_stepped_stimulus_info_to_db(stimulus_parameter: dict, database):
        probe_values = frequency_stepped_insert_values(stimulus_parameter, is_default=0)
        ...
        if query_code == error_code.OK:
            for row in rows:
                parsed = parse_frequency_stepped_row(stimulus_row_to_dict(row))
