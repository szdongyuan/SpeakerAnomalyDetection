...
def a_weighting(f: np.ndarray) -> np.ndarray:
    ...
    r_a = num / (den + 1e-30)
    return 20 * np.log10(r_a + 1e-30) + 2.0


def c_weighting(f: np.ndarray) -> np.ndarray:
    """C 计权修正值 (dB)"""
    f = np.asarray(f, dtype=float)
    f2 = f ** 2
    num = 12194.0 ** 2 * f2
    ...
    r_c = num / (den + 1e-30)
    return 20 * np.log10(r_c + 1e-30) + 0.062


def z_weighting(f: np.ndarray) -> np.ndarray:
    """Z 计权 (无计权)"""
    return np.zeros_like(f, dtype=float)


WEIGHTING_FUNCTIONS = {"A": a_weighting, "C": c_weighting, "Z": z_weighting}


def get_weighting_fn(name: str = "Z"):
    name = name.upper()
    if name in ("NONE", "Z（NONE）"):
        name = "Z"
    return WEIGHTING_FUNCTIONS.get(name, z_weighting)
