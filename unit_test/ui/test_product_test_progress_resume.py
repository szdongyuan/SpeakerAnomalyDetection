...
def test_play_button_and_click_use_same_serial_switch(factory, enabled):
    host = factory()
    serial_conditions(host)
    host._serial_trigger_config["enabled"] = enabled
    assert host._offer_product_test_resume()
    host.start_this_play = Mock()
    SequenceWidgetUiOpsMixin.update_player_btn_is_paused(host)
    assert host.player_btn.isEnabled() is not enabled
    host.on_clicked_player_btn()
    assert host.start_this_play.called is not enabled


def configure_serial_dialog(host, monkeypatch, enabled, during_dialog=None):
    current = dict(host._serial_trigger_config)
    next_config = dict(current, enabled=enabled)
    monkeypatch.setattr(
        serial_ops.LoadUiConfig, "load_serial_discrete_input_config", lambda: (0, current),
    )
    save = Mock(return_value=True)
    monkeypatch.setattr(serial_ops.LoadUiConfig, "save_serial_discrete_input_config", save)
    def accept():
        if during_dialog:
            during_dialog()
        return "save", next_config

    dialog = Mock(return_value=SimpleNamespace(exec=accept))
    monkeypatch.setattr(serial_ops, "SerialDiscreteInputConfigDialog", dialog)
    host.hw_manager = Mock()
    host.hw_manager.get_serial_discrete_input_status.return_value = {}
    host.hw_manager.start_serial_discrete_input_listener.return_value = {"ok": True}
    host.serial_trigger_btn = host.toolsbar.serial_trigger_btn
    host.update_player_btn_is_paused = lambda: SequenceWidgetUiOpsMixin.update_player_btn_is_paused(host)
    return dialog, save


def test_serial_off_continues_next_gear_and_on_keeps_manual_progress(factory, monkeypatch):
    host = factory()
    frames = serial_conditions(host)
    host._serial_trigger_config["enabled"] = True
    begin_partial_round(host)
    group_id = host._manual_product_condition_group_id
    host._serial_product_port_index = 0
    host._serial_product_waiting_port_idle = True
    host.start_this_play = Mock()

    def while_open():
        host.on_serial_full_frame_received({"raw_hex": frames[1]})
        assert not host._can_prepare_recording_workflow()
        host.start_this_play.assert_not_called()

    configure_serial_dialog(host, monkeypatch, False, while_open)
    ...
    assert host._manual_product_condition_group_id == group_id
    assert host._manual_product_condition_index == 1
    host.on_serial_full_frame_received({"raw_hex": frames[1]})
    host.start_this_play.assert_not_called()
    host.on_clicked_player_btn()
    host.start_this_play.assert_called_once()
    assert host._get_active_product_condition_key() == "b"
    complete_condition(host, "b")
    host._advance_manual_product_condition_cycle_after_recording()
    assert not host._refresh_serial_product_port_state()
    assert host._manual_product_condition_index == 2

    configure_serial_dialog(host, monkeypatch, True)
    host.on_serial_trigger_btn_clicked()
    ...
    assert host._manual_product_condition_group_id == group_id
    host._start_serial_product_condition = Mock(return_value=True)
    host.on_serial_full_frame_received({"raw_hex": frames[1]})
    host._start_serial_product_condition.assert_not_called()
    host.on_serial_full_frame_received({"raw_hex": frames[2]})
    host._start_serial_product_condition.assert_called_once_with(frames[2])


@pytest.mark.parametrize("busy", ["recording", "analysis"])
def test_busy_test_cannot_change_serial_switch(factory, monkeypatch, busy):
    host = factory()
    host._serial_trigger_config = {"enabled": True}
    dialog, save = configure_serial_dialog(host, monkeypatch, False)
    host.player_status_flag = busy == "recording"
    host._analysis_has_pending_tasks = lambda: busy == "analysis"
    monkeypatch.setattr(serial_ops.QMessageBox, "information", Mock())
    host.on_serial_trigger_btn_clicked()
    dialog.assert_not_called()
    save.assert_not_called()
    assert host._serial_trigger_config["enabled"]


def test_serial_off_releases_completed_round_waiting_for_close_code(factory, monkeypatch):
    host = factory()
    serial_conditions(host)
    host._serial_trigger_config["enabled"] = True
    begin_partial_round(host)
    for key in ("b", "c"):
        complete_condition(host, key)
    host._serial_product_waiting_for_close = True
    configure_serial_dialog(host, monkeypatch, False)
    host.on_serial_trigger_btn_clicked()
    assert not host._serial_product_waiting_for_close
    assert host._manual_product_condition_group_id == ""
    assert not host._analysis_round_config_locked
    assert host.player_btn.isEnabled()
    assert all((host.directory / f"{key}.wav").exists() for key in ("a", "b", "c"))


def test_serial_off_late_error_status_preserves_manual_round(factory):
    host = factory()
    begin_partial_round(host)
    group_id = host._manual_product_condition_group_id
    host._serial_trigger_config = {"enabled": False}
    host.serial_trigger_btn = host.toolsbar.serial_trigger_btn
    host.on_serial_trigger_status_changed({"enabled": True, "error": "late error"})
    assert host._manual_product_condition_group_id == group_id
    assert host._manual_product_condition_completed_keys == {"a"}


def test_restart_with_serial_off_enables_play_and_resumes_waiting_port(factory, monkeypatch):
    original = factory()
    serial_conditions(original)
    begin_partial_round(original)
    original._serial_product_waiting_port_idle = True
    original._serial_product_port_index = 0
    original._save_product_test_progress_before_exit()
    reopened = factory()
    serial_conditions(reopened)
    assert reopened._offer_product_test_resume()
    configure_serial_dialog(reopened, monkeypatch, False)
    config = dict(reopened._serial_trigger_config, enabled=False)
    monkeypatch.setattr(serial_ops.LoadUiConfig, "load_serial_discrete_input_config", lambda: (0, config))
    serial_ops.SequenceWidgetSerialTriggerOpsMixin.init_serial_trigger_runtime(reopened)
    assert reopened.player_btn.isEnabled()
