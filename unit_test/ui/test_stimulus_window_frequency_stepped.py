...
from base.soundcard_audio_processor import _alignment_reference_from_stimulus
...
@pytest.fixture
def local_tmp_path():
    path = Path(".pytest-task6") / uuid4().hex
...
def window_factory(qapp, monkeypatch):
    ...
    monkeypatch.setattr(stimulus_window.model_consts, "STORED_STIMULUS_PATH", ".pytest-task3")
...
def test_step_sc_end_to_end_json_db_runtime_and_analysis_use_retained_schedule(
    ...
    playback, sample_rate, save_path = stimulus_resolver._generate_and_save_stimulus(detail)
    ...
    thd_x, _, thd = AudioThdFrequencyResponseAnalysis()._calculate_thd_three_phase(
        recording,
        sample_rate,
