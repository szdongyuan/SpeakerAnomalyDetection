...
def test_default_owned_bridge_shutdown_on_close_or_destroy(setup, ui_qapp, monkeypatch, destroy):
    from PyQt5 import sip
    service = SimpleNamespace(busy=False, shutdown=mock.Mock())
    monkeypatch.setattr(calibration, "RecordingService", lambda: service)
    widget = calibration.InputCalibration(device_info(), [7],
        ve_profile_store=setup.profiles, ve_calibration_store=setup.calibrations)
    bridge = widget._get_recording_bridge()
...
def test_injected_bridge_survives_input_destruction(setup):
    from PyQt5 import sip
    widget = calibration.InputCalibration(device_info(), [7], recording_bridge=setup.bridge,
        ve_profile_store=setup.profiles, ve_calibration_store=setup.calibrations)
    setup.bridge.shutdown = mock.Mock()
    sip.delete(widget)
    setup.bridge.shutdown.assert_not_called()


@pytest.mark.parametrize("close_first", [False, True])
def test_default_owned_real_service_destroy_reaps_active_worker(setup, ui_qapp, monkeypatch, caplog, close_first):
    from PyQt5 import sip
    from base.recording_service import RecordingService
    from unit_test.ui.test_recording_process_integration import pump
    service = RecordingService(
        backend_factory="unit_test.base.test_ve3668n_calibration:sine_capture_dependencies",
        backend_options={"parent_pid": os.getpid()})
    monkeypatch.setattr(calibration, "RecordingService", lambda: service)
    device = device_info(input_config=input_config(48000))
    setup.profiles.set_sample_rate(device, 48000, setup.calibrations)
    widget = calibration.InputCalibration(device, [7],
        ve_profile_store=setup.profiles, ve_calibration_store=setup.calibrations)
    try:
        assert widget.clicked_calibration()
        session = widget.streaming_processor.session
        pump(ui_qapp, lambda: session.worker_pid is not None)
        process = service._worker.process
        assert process.is_alive() and not session.released.is_set()
        if close_first:
            widget.close_recording()
        sip.delete(widget)
        pump(ui_qapp, service.closed.is_set, timeout=10)
        pump(ui_qapp, lambda: not any(t.is_alive() for t in service.threads), timeout=10)
        assert process._closed  # Process.close() requires that the process has exited.
        assert "callback failed" not in caplog.text
        assert not service.is_path_leased(session.request.path)
        assert not Path(session.request.path).exists()
        assert setup.calibrations.get_record(device, 7) is None
    finally:
        if not sip.isdeleted(widget):
            widget.close_recording()
            sip.delete(widget)
