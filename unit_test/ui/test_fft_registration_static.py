...
def test_sequence_marks_fft_as_v2pa_required():
    ...
    values = {element.value for element in assigned_sets[0].elts if isinstance(element, ast.Constant)}
...
def test_runtime_registers_and_dispatches_fft_analysis():
    ...
    sequence_source = SEQUENCE_WIDGET_PATH.read_text(encoding="utf-8")
