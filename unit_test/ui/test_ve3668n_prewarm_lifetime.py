...
def test_main_window_requires_explicit_lifetime_and_stores_injected_instance():
    from main_window import MainWindow

    with pytest.raises(TypeError, match="ve_prewarm_lifetime"):
        MainWindow.__init__(object())
    with pytest.raises(TypeError, match="ve_prewarm_lifetime"):
        MainWindow.__init__(object(), ve_prewarm_lifetime=None)
    ...
    assignments = [
        node for node in ast.walk(main_window)
        if isinstance(node, ast.Assign)
        and any(isinstance(target, ast.Attribute)
                and isinstance(target.value, ast.Name)
                and target.value.id == "self"
                and target.attr == "ve_prewarm_lifetime"
                for target in node.targets)
    ]
    assert len(assignments) == 1
    assert isinstance(assignments[0].value, ast.Name)
    assert assignments[0].value.id == "ve_prewarm_lifetime"
