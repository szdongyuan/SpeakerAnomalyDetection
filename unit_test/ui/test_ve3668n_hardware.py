...
def _configure_main_calibration_admission(
    ...
    for name in (
        "_recording_admission_config_snapshot",
        "_unfinished_recording_analysis_identifiers",
...
def test_main_calibration_action_opens_during_eligible_result_finalization(
    ...
    assert window.hardware_action_calibration.isEnabled()
    window.hardware_action_calibration.trigger()

    assert len(constructed) == 1
    assert constructed[0]["recording_bridge"] is main_window_harness.bridge
