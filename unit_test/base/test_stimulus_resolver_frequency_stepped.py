...
def _local_tmp_dir():
    path = Path(".pytest-task2") / uuid4().hex
...
def test_supported_aliases_normalize_and_retained_frequencies_regenerate_authoritative_metadata(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))

    detail = {"stimulus_info": _runtime_info(stimulus_method="Frequency Stepped")}

    data, sample_rate, save_path = stimulus_resolver._generate_and_save_stimulus(detail)
...
def test_frequency_stepped_saved_wav_path_is_not_authoritative(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))
...
def test_cached_same_rate_schedule_is_reused_only_with_matching_provenance(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))
    ...
    stimulus_resolver._generate_and_save_stimulus(detail)
...
def test_cross_rate_cached_schedule_rebuilds_from_retained_frequencies(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))
...
def test_insufficient_frequency_stepped_metadata_raises_value_error(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))
    detail = {
        "stimulus_info": {
            "stimulus_method": "frequency_stepped",
...
def test_frequency_stepped_runtime_missing_sample_rate_raises_value_error(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))
    info = _runtime_info()
...
def test_runtime_extra_fields_are_filtered_before_generator_call(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))
    calls = []
...
def test_stale_mode_conflicts_are_repaired_before_generation(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))
...
def test_retained_frequencies_take_precedence_over_scalar_reconstruction_fields(monkeypatch):
    monkeypatch.setattr(stimulus_resolver.model_consts, "STORED_STIMULUS_PATH", str(_local_tmp_dir()))
    detail = {
        "stimulus_info": _runtime_info(
            start_freq=20.0,
