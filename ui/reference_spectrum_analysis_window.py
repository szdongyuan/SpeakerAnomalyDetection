...
class ReferenceSpectrumCompareWindow(QWidget):
    def __init__(self, title_name):
        ...
        self.axis_mode_button = QPushButton()
    ...
    def _get_or_create_plot_cache(plot_widget: pg.PlotWidget) -> dict:
        ...
        cache = {
            ...
            "lower_curve": plot_widget.plot(
                [], [], pen=mkPen(color=LIMIT_CURVE_COLOR, width=1.6, style=Qt.DashLine)
            ),
            "out_of_range_curve": plot_widget.plot(
                [], [], pen=mkPen(color=OUT_OF_RANGE_CURVE_COLOR, width=3.0)
            ),
        }
    ...
    def _render_all_channel_results(self):
        ...
        for card_info, compare_result in zip(self.channel_cards, self._compare_results):
            card_info["title_label"].setText(
                f"通道：{self._resolve_channel_label(compare_result.channel_index)}"
            )
    ...
    def calculate_reference_spectrum(self):
        try:
            ...
            QMessageBox.warning(self, "提示", str(e)[:200])
    ...
    def _plot_channel_result(self, plot_widget: pg.PlotWidget, compare_result):
        ...
        cfg = self.analysis_config or {}
        show_band_highlight = bool(cfg.get("use_custom_band", True)) and bool(
            cfg.get("highlight_analysis_band", True)
        )
