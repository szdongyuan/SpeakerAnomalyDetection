...
class SequenceWidgetSerialTriggerOpsMixin:
    ...
    def on_serial_trigger_btn_clicked(self):
        ...
        if action == "test":
            restart_config = None
            test_port = str((self._serial_trigger_config.get("serial_settings", {}) or {}).get("port", "") or "")
            ...
            ret = self.hw_manager.test_serial_discrete_input_connection(self._serial_trigger_config)
            ...
            if ret.get("ok"):
                raw_hex = ret.get("raw_hex", "") or "无响应数据"
                QMessageBox.information(self, "测试连接", f"{ret.get('message', '成功')}\n\n最近接收码: {raw_hex}")
            else:
                QMessageBox.warning(self, "测试连接失败", ret.get("message", "未知错误"))
            return
