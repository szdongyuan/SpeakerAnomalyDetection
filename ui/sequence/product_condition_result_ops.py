class SequenceWidgetProductConditionResultOpsMixin:
    ...
    def _collect_product_condition_records(self, group_id):
        ...
        restored_records = getattr(self, "_condition_record_cache", {}) or {}
        for condition_key, record in restored_records.items():
            if not isinstance(record, dict):
                continue
            if str(record.get("source_type") or "").strip() != "restored":
                continue
            if str(record.get("group_id") or "").strip() != group_id:
                continue
            key = str(
