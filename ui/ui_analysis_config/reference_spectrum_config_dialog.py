...
class ReferenceSpectrumConfigWindow(SemanticAnalysisConfigDialogBase):
    ...
    def __init__(self, config_manager, model_type, available_channels: Optional[List[int]] = None):
        ...
        self._channel_name_inputs: dict[int, LineEdit] = {}
    ...
    def _create_reference_group(self):
        ...
        source_path_row.addWidget(Label("参考样本"))
        self.reference_source_path_edit = LineEdit()
    ...
    def _create_band_group(self):
        group = GroupBox("分析频段（可选）")
        layout = QVBoxLayout()

        self.custom_band_checkbox = CheckBox("自定义分析频段")
    ...
    def _create_threshold_group(self):
        group = GroupBox("阈值判定（可选）")
        layout = QVBoxLayout()

        self.enable_threshold_checkbox = CheckBox("启用阈值判定")
        ...
        lower_row.addWidget(Label("下偏移 (dB)"))
        self.lower_offset_spinbox = DoubleSpinBox()
        ...
        upper_row.addWidget(Label("上偏移 (dB)"))
        self.upper_offset_spinbox = DoubleSpinBox()
    ...
    def _create_channel_name_group(self):
        self.channel_name_group = GroupBox("通道名称（可选）")
        group_layout = QVBoxLayout()
        self.custom_channel_name_checkbox = CheckBox("自定义通道名称")
    ...
    def _create_advanced_group(self):
        ...
        window_row.addWidget(Label("窗函数"))
        self.window_combo_box = ComboBox()
        ...
        nperseg_row.addWidget(Label("分段长度"))
        self.nperseg_combo_box = ComboBox()
        ...
        overlap_row.addWidget(Label("重叠比例"))
        self.overlap_combo_box = ComboBox()
    ...
    def _rebuild_channel_name_rows(self, channel_count: int):
        ...
        for channel_index in range(channel_count_value):
            ...
            row_layout.addWidget(Label(f"CH{channel_index + 1} 名称"))
            line_edit = LineEdit()
    ...
    def _on_reference_file_browse(self):
        ...
        try:
            ...
            self._reference_audio_meta = None
            self._refresh_reference_info_labels(None)
            MessageBox.warning(self, "提示", f"参考音频读取失败：{str(e)[:200]}")
        self._refresh_reference_data_status()
    ...
    def _ensure_reference_data_ready(self, *, show_error_message: bool = False) -> bool:
        ...
        try:
            ...
            if show_error_message:
                MessageBox.warning(self, "提示", f"参考样本准备失败：{str(e)[:200]}")
            return False

    def validate(self, *, save_reference_sample_data: bool = False) -> bool:
        source_path = self.reference_source_path_edit.text().strip()
        if not source_path:
            MessageBox.warning(self, "提示", "请先导入参考样本")
            return False
        if not os.path.exists(source_path):
            MessageBox.warning(self, "提示", "参考样本文件不存在，请重新选择")
            return False
        if self.custom_band_checkbox.isChecked() and self.end_freq_spinbox.value() <= self.start_freq_spinbox.value():
            MessageBox.warning(self, "提示", "分析频段配置错误，请检查起始频率和结束频率")
            return False
        if (
            self.enable_threshold_checkbox.isChecked()
            and self.lower_offset_spinbox.value() > self.upper_offset_spinbox.value()
        ):
            MessageBox.warning(self, "提示", "阈值范围配置错误，请检查上下偏移")
            return False
