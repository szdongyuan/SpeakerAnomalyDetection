...
class LoudnessConfigPanel(QWidget):
    ...
    def _create_algorithm_group(self):
        ...
        icon = QIcon(DEFAULT_DIR + "ui/ui_pic/folder/folder-s.png")
...
class LoudnessConfigWindow(SemanticAnalysisConfigDialogBase):
    ...
    DEFAULT_CONFIG = {
        ...
        "advanced": {
            "show_specific_loudness_heatmap": False,
            "save_specific_loudness_npz": False,
            "specific_loudness_colormap": "viridis",
            "curve_y_unit": "sone",
            "specific_loudness_profile_mode": "steady_average",
            "specific_loudness_exceedance_threshold_sone_per_bark": 0.0,
            "specific_loudness_exceedance_ref_line": "ref1",
            "output_time_resolution_ms": 2.0,
            "stationary_frame_duration_s": 0.1,
            "stationary_overlap_percent": 50.0,
            "stationary_hop_duration_s": 0.05,
            "curve_y_axis_zero_based": True,
            "analysis_time_range_enabled": False,
            "analysis_start_time_sec": 0.0,
            "analysis_end_time_sec": 0.0,
            "background_noise_processing_enabled": False,
            "background_noise_file_path": "",
            "background_noise_algorithm": "spectral_subtraction_audio",
            "background_noise_n_fft": 4096,
            "background_noise_hop_size": 1024,
            "background_noise_oversubtraction_factor": 1.0,
            "background_noise_spectral_floor": 0.02,
            "background_noise_min_gain_db": -20.0,
            "background_noise_frequency_smoothing_bins": 3,
            "background_noise_gain_time_smoothing": 0.6,
        },
        "limit_checked": False,
    ...
    @classmethod
    def merge_with_defaults(cls, config):
        source = config if isinstance(config, dict) else {}
        merged = copy.deepcopy(cls.DEFAULT_CONFIG)
        for key, value in source.items():
            if key in {"display", "save", "advanced"} and isinstance(value, dict):
    ...
    def __init__(self, config_manager, model_type, available_channels: Optional[List[int]] = None):
        ...
        self.load_config = self.merge_with_defaults(
    ...
    def on_restore_default_btn_clicked(self):
        self.load_config = self.merge_with_defaults(
            self.config_manager.load_config().get(self.model_type, {})
        )
