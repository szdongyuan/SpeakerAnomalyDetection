...
class LooseParticle(QWidget):
    ...
    def plot_graph(self, amplitude):
        ...
        self.lp_graph.plot(signal_duration, amplitude)
    ...
    def detect_peaks(self, filtered_db, max_threshold, min_threshold , min_check_duration, max_check_duration, sampling_rate, signal_duration):
        ...
        for peak in peaks:
            if first_iteration:
                ...
                if (peak - last_peak) * 4 * 1000 / sampling_rate < min_check_duration:
                    continue
            start_index = peak
            last_peak = peak
            end_index = start_index
            while end_index + 1 < len(filtered_db) and filtered_db[end_index + 1] > min_threshold:
                if filtered_db[end_index + 1] > max_threshold:
                    start_index = end_index + 1
                    last_peak = end_index + 1
                    current_out_range = []
                if filtered_db[end_index + 1] < min_threshold:
                    break
